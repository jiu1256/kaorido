...
- notes: ノートとして明示されたリスト/表記(TOP/MIDDLE等のラベルと素材名の
  対応が明確なもの)のみ抽出。創作禁止。
  - ラベルのマッピング: TOP/トップ → top、MIDDLE/ミドル → middle、
    LAST/ラスト/BASE/ベース → base
  - 説明文・スタッフコメント・レビュー等の文章中に素材名が登場するだけの場合、
    そこからの推測分類は禁止。notes は全て空配列にして warnings に
    「ノート表記なし」と記載
  - UIの凡例やチャートの見出しだけにラベルがあり、素材名との対応が
    読み取れない場合も「ノート表記なし」として扱う
  - 素材名のリスト表記はあるがラベルが無い場合のみ、全て middle に入れて
    warnings に記載
  - 負例: 説明文「柑橘の爽やかさとフローラルな甘さが広がり、サンダルウッドと
    カシミヤウッドがあたたかみを添えます」のような文章しか無い場合、
    notes は {"top":[],"middle":[],"base":[]}（文章からの振り分けは創作にあたる）
  - notes 出力前の自己チェック: 各素材名について「ページ上で TOP/MIDDLE/LAST 等の
    ラベルの直下・直後に素材名として列挙されているか」を確認し、
    確認できないものは notes に含めないこと
- variants:
